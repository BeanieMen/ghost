...
export function isSuccess<T>(state: OperationState<T>): state is OperationState<T> & { status: "success" } {
  return state.status === "success";
}

/** Type guard for error state */
export function isError<T>(state: OperationState<T>): state is OperationState<T> & { status: "error" } {
  return state.status === "error";
}

/** Type guard for loading state */
export function isLoading<T>(state: OperationState<T>): state is OperationState<T> & { status: "loading" } {
...
export function handleState<T, R>(
  ...
  switch (state.status) {
    ...
    case "success": return handlers.success(state.data, state.completedAt);
    case "error": return handlers.error(state.error, state.failedAt);
    default: {
